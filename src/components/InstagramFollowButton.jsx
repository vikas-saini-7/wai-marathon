...
const InstagramFollowButton = () => {
  ...
  return (
    <div>
      {isFollowing ? (
        <button onClick={handleFollowClick} className='bg-blue-500 text-white py-2 px-4 mt-4 rounded'>Get Wai Marathon Photos</button>
      ) : (
        <button onClick={handleFollowClick} disabled className='bg-blue-500 text-white py-2 px-4 mt-4 rounded opacity-50 cursor-not-allowed'>First follow on Instagram</button>
      )}
    </div>
  );
};
